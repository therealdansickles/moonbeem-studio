...
//
// Session Replay is deliberately OFF (wizard answer). Server-side scrubbing
// lives in sentry.server.config.ts / sentry.edge.config.ts; the browser lane
// sends no auth headers or request bodies of ours.
...
Sentry.init({
  ...
  // Explicitly OFF, not by omission (deprecated in SDK v10 for
  // `dataCollection`; an empty `dataCollection` block would make the SDK
  // ignore this flag, so none is set).
  sendDefaultPii: false,

  // Match the server posture: 10% of pageload/navigation traces.
